...
public class ARController : MonoBehaviour
{
    ...
    private void SpawnLevel(TrackableHit hit)
    {
        if (_levelSpawned) return;

        var andyObject = Instantiate(level, hit.Pose.position, hit.Pose.rotation);
        andyObject.transform.localScale = new Vector3(modelScale, modelScale, modelScale);
